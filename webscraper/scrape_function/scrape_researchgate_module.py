...
def scrape_researchgate(author):
    ws_db = author.website_set.get(website_name='Research Gate')
    webpage = ws_db.website_link + ws_db.website_author_string + '/'  # Armin_Doerry
    ...
    df_main = pd.DataFrame(columns=['Name', 'Hyperlink', 'PDF',
                                    'Article Type', 'Publication Date', 'Publication', 'Abstract'])

    for targetPage in targetPage_list:
        print(targetPage)
        pageContent = requests.get(targetPage, timeout=5)
        soup = BeautifulSoup(pageContent.content, 'html.parser')
        headline_list, hyperlink_list, fullText_list, articleType_list, datePublished_list, publication_list, description_list = scrapePage(
            soup)
...
def scrapePage(soup):
    ...
    for research_item in soup.findAll('div', attrs={"class": "gtm-research-item"}):
        ...
        metaRight = research_item.find(
            'div', attrs={"class": "nova-v-publication-item__meta-right"})
        if metaRight:
        ...
        itemDescrp_attr = research_item.select(
    ...
    return headline_list, hyperlink_list, fullText_list, articleType_list,\
        datePublished_list, publication_list, description_list
